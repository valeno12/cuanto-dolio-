...
const confirmPayment = async () => {
    ...
    try {
        const response = await fetch(`/${props.roomCode}/settlements/${payingSettlement.value.id}/pay`, {
            method: 'POST',
            headers: {
...
<template>
    <div class="space-y-6 pb-24">
        ...
        <template v-else-if="!isLocked">
            <!-- Empty State: Room not locked -->
            <div class="flex flex-col items-center justify-center py-16 text-center">
                <div class="mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-slate-800 ring-1 ring-white/10">
                    <span class="text-4xl">🔓</span>
                </div>
                <h3 class="mb-2 text-xl font-bold text-white">Sala abierta</h3>
                <p class="mx-auto max-w-[280px] text-sm leading-relaxed text-slate-400">
                    Cerrá la sala cuando terminen de cargar gastos para ver quién le debe a quién.
                </p>
            </div>
        </template>
        ...
        <template v-else>
            <!-- Hero Status Card -->
            <div class="rounded-3xl border border-white/5 bg-gradient-to-br from-slate-800 to-slate-900 p-6 shadow-xl">
                <div class="mb-4 text-center">
                    <p class="text-sm font-medium tracking-wider text-slate-400 uppercase">Tu Balance</p>
                    <div v-if="totalIOwe > 0">
                        <h2 class="text-4xl font-bold text-red-500">{{ formatCurrency(totalIOwe) }}</h2>
                        <p class="mt-1 text-slate-400">Tenés que pagar</p>
                    </div>
                    <div v-else-if="totalOwedToMe > 0">
                        <h2 class="text-4xl font-bold text-green-500">{{ formatCurrency(totalOwedToMe) }}</h2>
                        <p class="mt-1 text-slate-400">Te deben</p>
                    </div>
                    <div v-else>
                        <h2 class="text-4xl font-bold text-secondary-500">🥳</h2>
                        <p class="mt-1 text-slate-400">Estás al día</p>
                    </div>
                </div>

                <!-- Primary Action if I owe -->
                <div v-if="iOwe.length > 0" class="mt-6 flex flex-col gap-3">
                    <div v-for="settlement in iOwe" :key="settlement.id" class="rounded-xl bg-white/5 p-4">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div v-if="settlement.to.payment_alias" class="flex items-center gap-2">
                                        <span class="font-mono text-xs text-slate-400">{{ settlement.to.payment_alias }}</span>
                        ...
                        <button
                            ...
                            class="w-full rounded-lg bg-primary-500 py-3 font-medium text-white shadow-lg shadow-primary-500/25 transition-all active:scale-95"
                        >
                            Pagar ahora
                        </button>
                        <div v-else class="rounded-lg bg-green-500/20 py-2 text-center text-sm font-medium text-green-400">
            ...
            <div v-if="theyOweMe.length > 0 || virtualSettlements.length > 0" class="space-y-4">
                <h3 class="px-2 font-medium text-slate-400">Otros Movimientos</h3>
                    ...
                    >
                        <div class="relative z-10 flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="leading-tight font-bold text-white">{{ settlement.from.name }}</p>
                            ...
                            <div class="text-right">
                                ...
                                <span
                                    v-if="settlement.is_paid"
                                    class="inline-flex items-center gap-1 rounded-full bg-green-500/10 px-2 py-0.5 text-[10px] font-bold tracking-wide text-green-400 uppercase"
                                >
                                    <span class="h-1.5 w-1.5 rounded-full bg-green-400"></span> Pagado
                                </span>
                                <span
                                    v-else
                                    class="inline-flex items-center gap-1 rounded-full bg-amber-500/10 px-2 py-0.5 text-[10px] font-bold tracking-wide text-amber-500 uppercase"
                                >
                                    <span class="h-1.5 w-1.5 animate-pulse rounded-full bg-amber-500"></span> Pendiente
                                </span>
                            </div>
                        </div>
                        <!-- Background decoration -->
                        <div
                            class="absolute -top-4 -right-4 h-20 w-20 rounded-full bg-gradient-to-br from-secondary-500 to-primary-500 opacity-5 blur-xl"
                        ></div>
                    </div>
                </div>
            ...
            <div class="flex gap-3">
                <a
                    :href="`/${roomCode}/export-pdf`"
                    class="flex flex-1 items-center justify-center gap-2 rounded-xl bg-white/10 py-4 font-bold text-white transition-all hover:bg-white/20 active:scale-95"
                >
                    <span>📄</span> Exportar PDF
                </a>
                <button
                    @click="shareWhatsApp"
                    class="flex flex-1 items-center justify-center gap-2 rounded-xl bg-[#25D366] py-4 font-bold text-white shadow-lg transition-all active:scale-95"
                >
                    <span>📲</span> WhatsApp
                </button>
            </div>
        ...
        <Teleport to="body">
            <div v-if="showPaymentModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
                <div class="w-full max-w-sm rounded-3xl border border-slate-800 bg-slate-900 p-6 shadow-2xl">
                    <h3 class="mb-6 text-center text-xl font-bold text-white">Confirmar Pago</h3>
                    ...
                    <div class="mb-8 space-y-3">
                        <button
                            ...
                            :class="[
                                ...
                                paymentMethod === 'transfer' ? 'border-primary-500 bg-primary-500/10' : 'border-slate-700 hover:border-slate-600',
                            ]"
                        >
                        ...
                        <button
                            ...
                            :class="[
                                'flex w-full items-center gap-4 rounded-xl border p-4 transition-all',
                                paymentMethod === 'cash' ? 'border-primary-500 bg-primary-500/10' : 'border-slate-700 hover:border-slate-600',
                            ]"
                        >
                            <span class="text-2xl">💵</span>
                            <div class="text-left">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <button @click="showPaymentModal = false" class="rounded-xl bg-slate-800 py-3 font-medium text-slate-300 hover:bg-slate-700">
                            Cancelar
                        </button>
                        <button
